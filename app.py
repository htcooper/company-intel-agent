...
def run_pipeline(company: str, api_key: str, provider: str, status_callback) -> str:
    ...
    brief = brief.rstrip() + f"\n\n*Run date: {date_str}*"
    brief = brief.rstrip() + f"\n*AI model: {config.display_name} ({config.research_model})*"
...
def _parse_brief(brief: str) -> dict:
    ...
    result = {"doing": "", "saying": "", "hiring": "", "bottom": "", "sources": "", "date": "", "attribution": "", "ai_model": ""}

    # Strip leading markdown headings (e.g., "## Stripe" in EXAMPLE_BRIEF)
    text = re.sub(r"^\s*#{1,6}\s+[^\n]+\n+", "", brief.strip())

    # Pull trailing metadata out of the text BEFORE running section regexes,
    # so section lookaheads can't accidentally consume them. The italic
    # asterisks are optional — Anthropic emits them, OpenAI sometimes doesn't.
    attr_m = re.search(r"\*?\s*(Generated by[^\n*]+?)\s*\*?\s*(?:\n|$)", text)
    if attr_m:
        result["attribution"] = attr_m.group(1).strip()
        text = text[:attr_m.start()] + text[attr_m.end():]

    date_m = re.search(r"\*?\s*Run date:\s*([^\n*]+?)\s*\*?\s*(?:\n|$)", text)
    if date_m:
        result["date"] = date_m.group(1).strip()
        text = text[:date_m.start()] + text[date_m.end():]

    ai_m = re.search(r"\*?\s*AI model:\s*([^\n*]+?)\s*\*?\s*(?:\n|$)", text)
    if ai_m:
        result["ai_model"] = ai_m.group(1).strip()
        text = text[:ai_m.start()] + text[ai_m.end():]

    sources_m = re.search(r"\*\*Sources\*\*\s*\n(.+)\Z", text, re.DOTALL)
    if sources_m:
        result["sources"] = sources_m.group(1).strip()
        text = text[:sources_m.start()]
